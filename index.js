...
app.get("/methods",authMiddleware, async (req, res) => {
  try {
    const qry = await pool.query(
      `SELECT main_data.rid,main_data.id,main_data.location,main_data.component,main_data.component_type,resource_data.id,resource_data.reuse,resource_data.reduce,resource_data.recycle,resource_data.link FROM main_data JOIN resource_data ON main_data.rid = resource_data.id or main_data.component_type=resource_data.component_type;`
    );
    res.json(qry.rows);
...
app.post("/methodsmain", async (req, res) => {
  ...
  try {
    ...
    const qry = await pool.query(
      `INSERT INTO main_data(location,component,component_type,rid) values($1,$2,$3,(select id from resource_data where component_type = '${componentType}'))`,
...
app.put("/ures/:id", async (req, res) => {
  try {
    ...
    const qry = await pool.query(
      "update resource_data set component_type=$1,reuse=$2,reduce=$3,recycle=$4,link=$5 where id =$6",
